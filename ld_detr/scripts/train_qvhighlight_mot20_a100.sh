...
#SBATCH --job-name=train_with_mot20_a100
...
t_feat_type=clip 
results_root=/home/atuin/v100dd/v100dd19/ld-detr/outputs/results_mot20
exp_id=exp
...
if [[ ${v_feat_types} == *"slowfast"* ]]; then
  v_feat_dirs+=(${feat_root}/slowfast_features_mot20)
  (( v_feat_dim += 2304 ))  # double brackets for arithmetic op, no need to use ${v_feat_dim}
fi
if [[ ${v_feat_types} == *"clip"* ]]; then
  v_feat_dirs+=(${feat_root}/clip_features_mot20)
  (( v_feat_dim += 512 ))
fi

# text features
if [[ ${t_feat_type} == "clip" ]]; then
  t_feat_dir=${feat_root}/clip_text_features_mot20/
  t_feat_dim=512
else
